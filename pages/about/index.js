...
exports.data = {
  title: 'About',
  color: 'pink',
  description: 'My name is Kristoffer Hedstrom. I\'m originally from Umeå, Sweden, but now live in Brooklyn. I make interactive things on the internet.',
  images: [{
  ...
  return (
    <div>
      <div className='about-photos'>
        {shuffle(images).slice(0, 3).map((image, i) =>
          <ImageLoader
              key={i}
      ...
      <div className='content-container'>
        ...
        <div className='row between-xs'>
          <div className='module col-xs-12 col-sm-8'>
            ...
            <p>{'I occasionally take on freelance projects. Feel free to get in touch if you have an interesting project that you want to collaborate on. Or if you just want to say '}<a className='highlight' href='mailto:hello@k-create.com?subject=hi'>{'hello'}</a>{', that’s cool too.'}</p>
          </div>
          <div className='module col-xs-12 col-sm-3'>
            <h2 className='sub-title'>{'Elsewhere'}</h2>
            <ul className='no-bullet-list social-links'>
              <li><a href='https://github.com/kristofferh' target='_blank'>{'Github'}</a></li>
              <li><a href='https://codepen.io/kristofferh/' target='_blank'>{'Codepen'}</a></li>
              <li><a href='http://kris.tumblr.com/' target='_blank'>{'Tumblr'}</a></li>
              <li><a href='https://twitter.com/_kris' target='_blank'>{'Twitter'}</a></li>
            </ul>
          </div>
        </div>
      </div>
